...
public sealed class MarkdownView : ContentView
{
	...
	public delegate void ImageTappedEventHandler(object sender, ImageEventArgs e);
	public event ImageTappedEventHandler OnImageTapped;

	public static readonly BindableProperty ImageTappedCommandProperty =
		BindableProperty.Create(nameof(ImageTappedCommand), typeof(ICommand), typeof(MarkdownView));
 ...
 BindableProperty.Create(nameof(TableRowBackgroundColor), typeof(Color), typeof(MarkdownView), Colors.White, propertyChanged: OnMarkdownTextChanged);
	...
	public static readonly BindableProperty BlockQuoteFontFaceProperty =
	 BindableProperty.Create(nameof(BlockQuoteFontFace), typeof(string), typeof(MarkdownView), propertyChanged: OnMarkdownTextChanged);
	...
	private View RenderParagraph(ParagraphBlock block)
	{
		...
		foreach (var inline in block.Inline)
		{
			...
			if (inline is LinkInline link && link.IsImage)
			{
				...
				// Attach tap handler so consumers can open the image full-screen.
				// Capture the URL in a local variable to avoid closure-over-loop issues.
				var tappedUrl = link.Url;
				var tapRecognizer = new TapGestureRecognizer();
				tapRecognizer.Tapped += (_, _) =>
				{
					var args = new ImageEventArgs { Url = tappedUrl };
					OnImageTapped?.Invoke(this, args);
					if (ImageTappedCommand?.CanExecute(tappedUrl) == true)
						ImageTappedCommand.Execute(tappedUrl);
				};
